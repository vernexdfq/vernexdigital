...
function NetworkLogo({
  ...
  if (!net) return null;
  const dim =
    size === "sm" ? "w-7 h-7 text-[10px]" : size === "lg" ? "w-10 h-10 text-sm" : "w-8 h-8 text-[11px]";
  return (
    <div
      className={`${dim} rounded-full flex items-center justify-center font-bold shrink-0 shadow-sm`}
      style={{ backgroundColor: net.brand, color: net.text }}
      title={net.name}
    >
      {net.id === "mtn" ? "MTN" : net.id === "airtel" ? "A" : net.id === "glo" ? "Glo" : "9m"}
    </div>
  );
}
