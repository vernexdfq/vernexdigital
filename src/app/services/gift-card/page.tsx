        ...
        <div className="px-4 pt-3 pb-4">
          ...
          {savedBanks.length === 0 ? (
            <div className="bg-white rounded-[12px] border border-[#E2E8F0] p-6 text-center">
              ...
              <button type="button" onClick={() => setView("addbank")} className="mt-3 h-10 px-4 rounded-full bg-[#1877F2] text-white text-sm font-semibold">Add Bank Account</button>
            </div>
          ) : (
            <div className="space-y-2">
              {savedBanks.map((b, i) => (
                <div key={i} className="bg-white rounded-[12px] border border-[#E2E8F0] p-3">
                  ...
                  <p className="text-xs text-[#64748B]">{b.acct} · {b.name}</p>
                </div>
              ))}
