...
export default function GiftCardPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#F0F7FF] to-white pb-24 overflow-y-auto">
      ...
      <header className="px-4 pt-4 pb-2">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-[#64748B] mt-0.5">Best rates · Fast settlement</p>
          </div>
        </div>
      </header>
        ...
        <div className="px-4 mt-4 grid grid-cols-2 gap-3 pb-24">
          {filteredCards.map((c) => (
            ...
            >
              ...
              <p className="mt-3 text-sm font-semibold text-[#0F172A] leading-tight">{c.name}</p>
              <p className="mt-1 text-xs font-medium text-[#1877F2]">{naira(c.rate)} / $</p>
            </button>
          ))}
        </div>
      )}
        ...
        <div className="px-4 mt-4 pb-24">
          <div className="flex gap-2 mb-4 overflow-x-auto">
            {(["ALL", "PENDING", "SUCCESS", "REJECTED"] as HistFilter[]).map((f) => (
              <button key={f} type="button" onClick={() => setHistFilter(f)} className={`h-8 px-3 rounded-full text-xs font-semibold shrink-0 ${histFilter === f ? "bg-[#1877F2] text-white" : "bg-white border border-[#E2E8F0] text-[#64748B]"}`}>{f}</button>
            ))}
          </div>
              ...
              {filteredOrders.map((o) => (
                <div key={o.id} className="bg-white rounded-[12px] border border-[#E2E8F0] p-4 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className={`text-[10px] font-semibold mt-0.5 ${o.status === "SUCCESS" ? "text-emerald-600" : o.status === "REJECTED" ? "text-red-500" : "text-amber-600"}`}>{o.status}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* Bottom nav: ONLY 3 items — blue sparkle first, History, Withdraw */}
