        ...
        <header className="sticky top-0 z-30 bg-gradient-to-b from-[#E8F1FF] to-[#F0F7FF] px-4 pt-3 pb-2">
          <div className="flex items-center gap-3">
            <button
              ...
              className="w-9 h-9 flex items-center justify-center text-[#0F172A]"
            >
              <ArrowLeft size={22} />
            </button>
        ...
        <div className="px-4 space-y-3">
          ...
          <div className="bg-white rounded-[14px] border border-[#E2E8F0] p-4 space-y-2 text-[12px]">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs text-[#1877F2] font-medium">Rules</span>
            </div>
            <div className="flex justify-between text-[#64748B]">
              <span>200/300/400/500</span>
              <span>1$ = {naira(rate)}</span>
            </div>
            <div className="flex justify-between text-[#64748B]">
              <span>100/150/250</span>
              <span>1$ = {naira(rate * 0.96)}</span>
            </div>
          </div>
          {entries.map((entry, i) => (
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setEntries((p) => p.map((x, j) => (j === i ? { ...x, face: v } : x)));
                    }}
                    placeholder="0"
                    inputMode="decimal"
                    className="mt-1 w-full h-11 px-2 rounded-[10px] bg-[#F8FAFC] border border-[#E2E8F0] text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    value={entry.qty}
                    onChange={(e) => {
                      const v = e.target.value.replace(/[^0-9]/g, "") || "1";
                      setEntries((p) => p.map((x, j) => (j === i ? { ...x, qty: v } : x)));
                    }}
                    inputMode="numeric"
                    className="mt-1 w-full h-11 px-2 rounded-[10px] bg-[#F8FAFC] border border-[#E2E8F0] text-sm text-center"
                  />
                </div>
                <div>
                  <label className="text-[11px] text-[#64748B]">You get</label>
                  <div className="mt-1 h-11 px-2 rounded-[10px] bg-[#EFF6FF] border border-[#BFDBFE] flex items-center justify-center text-sm font-semibold text-[#1877F2] tabular-nums">
                    {naira((Number(entry.face) || 0) * (Number(entry.qty) || 0) * rate)}
                  </div>
                </div>
              </div>
              <div>
                ...
                <input
                  value={entry.code}
                  onChange={(e) =>
                    setEntries((p) => p.map((x, j) => (j === i ? { ...x, code: e.target.value } : x)))
                  }
                  placeholder="Enter the card code"
                  className="mt-1 w-full h-11 px-3 rounded-[10px] bg-[#F8FAFC] border border-[#E2E8F0] text-sm"
                />
              </div>
              <div>
                ...
                <button
                  type="button"
                  className="mt-1.5 w-16 h-16 rounded-[12px] border-2 border-dashed border-[#CBD5E1] flex items-center justify-center text-[#94A3B8] bg-[#F8FAFC]"
                >
                  <Plus size={22} />
                </button>
              </div>
          ...
          <button
            type="button"
            onClick={() => setEntries((e) => [...e, { face: "", qty: "1", code: "" }])}
            className="w-full h-11 rounded-[12px] border border-dashed border-[#1877F2]/40 text-[#1877F2] text-sm font-medium"
          >
            + Send More Cards
          </button>
          <div className="flex items-center justify-between pt-2 pb-4">
            ...
            <button
              type="button"
              disabled={totalFace <= 0}
              onClick={submitSale}
              className="h-12 px-8 rounded-full bg-[#1877F2] text-white text-sm font-semibold disabled:opacity-40"
            >
              Submit
            </button>
          </div>
      ...
      <div className="min-h-screen bg-[#F8FAFC] pb-24">
        ...
        <div className="px-4 pt-5 space-y-4">
          <button
            ...
            className="w-full h-12 px-4 rounded-[12px] bg-white border border-[#E2E8F0] flex items-center justify-between text-sm"
          >
            <span className={bank ? "font-medium text-[#0F172A]" : "text-[#1877F2]"}>
          ...
          <input
            value={acct}
            onChange={(e) => setAcct(e.target.value.replace(/\D/g, "").slice(0, 13))}
            placeholder="Account Number (8-13 Digits)"
            inputMode="numeric"
            className="w-full h-12 px-4 rounded-[12px] bg-white border border-[#E2E8F0] text-sm"
          />
          <p className="text-[11px] text-[#94A3B8]">
            Please make sure your bank name and account number are correct.
          </p>
          {detecting && <p className="text-sm text-[#1877F2]">Detecting account name...</p>}
          {acctName && !confirmBank && (
            <p className="text-sm font-medium text-[#0F172A]">Account name: {acctName}</p>
          )}
        </div>
        {bankOpen && (
              ...
              <div className="px-4 pt-4 pb-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full h-11 pl-9 pr-3 rounded-full bg-[#F1F5F9] text-sm"
                    autoFocus
                  />
                </div>
              </div>
              <div className="overflow-y-auto px-2 pb-8">
                {filteredBanks.map((b) => (
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-3 py-3.5 rounded-[12px] hover:bg-[#F8FAFC] text-left"
                  >
                    <div
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center px-6">
            ...
            <div className="relative bg-white rounded-[16px] p-5 w-full max-w-sm shadow-xl">
              ...
              <p className="text-sm text-[#475569]">
                Are you sure you want to add bankno {acct} with account name {acctName}?
              </p>
              <div className="flex justify-end gap-4 mt-5">
                ...
                <button type="button" onClick={saveBank} className="text-sm font-semibold text-[#1877F2]">
                  OK
                </button>
              </div>
            </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-[#E2E8F0]">
          <button
            type="button"
            disabled={!bank || acct.length < 8 || !acctName}
            onClick={() => acctName && setConfirmBank(true)}
            className="w-full h-12 rounded-full bg-[#1877F2] text-white text-sm font-semibold disabled:opacity-40"
          >
            Submit
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#E8F1FF] via-[#F0F7FF] to-white pb-24">
      ...
      <div className="px-4 pt-3">
        <div className="bg-white/80 border border-[#BFDBFE] rounded-full px-4 py-2 flex items-center gap-2 overflow-hidden">
          ...
          <p className="text-[12px] text-[#0F172A] truncate">
            {trade.user} sold {trade.card} · {naira(trade.naira)}
          </p>
        </div>
      </div>
      ...
      <header className="px-4 pt-4 pb-2">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#1877F2]">
              Vernex Digital
            </p>
          ...
          <button
            ...
            className="h-9 px-3 rounded-full bg-[#1877F2] text-white text-xs font-semibold"
          >
            Add Bank
          </button>
        </div>
        <div className="mt-3 relative">
          ...
          <input
            ...
            className="w-full h-11 pl-9 pr-3 rounded-full bg-white border border-[#E2E8F0] text-sm"
          />
        </div>
      </header>
          ...
          {filteredOrders.length === 0 ? (
            ...
            <div className="space-y-2">
              {filteredOrders.map((o) => (
                <div
                  key={o.id}
                  className="bg-white rounded-[12px] border border-[#E2E8F0] p-3 flex items-center justify-between"
                >
                  <div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] font-semibold text-[#64748B]">{o.status}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="px-4 pt-3">
          ...
          {savedBanks.length === 0 ? (
            <div className="bg-white rounded-[12px] border border-[#E2E8F0] p-6 text-center">
              <p className="text-sm text-[#64748B]">No bank account yet</p>
              <button
                type="button"
                onClick={() => setView("addbank")}
                className="mt-3 h-10 px-4 rounded-full bg-[#1877F2] text-white text-sm font-semibold"
              >
                Add Bank Account
              </button>
            </div>
            ...
            <div className="space-y-2">
              {savedBanks.map((b, i) => (
                <div key={i} className="bg-white rounded-[12px] border border-[#E2E8F0] p-3">
              ...
              <button
                type="button"
                onClick={() => setView("addbank")}
                className="w-full h-10 rounded-[12px] border border-dashed border-[#1877F2]/40 text-[#1877F2] text-sm font-medium"
              >
                + Add another bank
              </button>
            </div>
          ...
          <button
            type="button"
            className="fixed bottom-20 left-4 right-4 h-12 rounded-full bg-[#1877F2] text-white text-sm font-semibold shadow-lg"
          >
            Withdraw
          </button>
        </div>
        ...
        <div className="max-w-lg mx-auto flex items-center justify-around h-16 px-2">
          <button
            ...
            className={`flex flex-col items-center gap-0.5 min-w-[64px] ${
              tab === "home" ? "text-[#1877F2]" : "text-[#64748B]"
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-0.5 min-w-[64px] ${
              tab === "history" ? "text-[#1877F2]" : "text-[#64748B]"
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-0.5 min-w-[64px] ${
              tab === "withdraw" ? "text-[#1877F2]" : "text-[#64748B]"
            }`}
          >
